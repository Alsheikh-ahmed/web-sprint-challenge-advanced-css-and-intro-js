...
*/
function getArtistByIndex(artists, position) {
  return "the artist at index " + position + " is " + artists[position].name
}
...
function get20s(artists) {
  ...
  for (let i = 0; i < artists.length; i++){
    let yearBorn = parseInt(artists[i].years)
    if (yearBorn >= 1900){
      dead20s.push(artists[i].name)
    }
  }
  return dead20s;
}
...
*/
function removeArtist(artists, position) {
  /*Your Code Here*/
...
function addArtist(artist) {
  let myInfo = {
    ...
    bio: 'Currently studying web design and hoping for a wonderful career in the future.'
  };
  artist.push(myInfo);
  return artist;
 }
